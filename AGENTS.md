...
Generated from `PROMPT.md`: everything below its `---`, verbatim. **`PROMPT.md` is the source of
truth** — change a rule there first, then regenerate this file. Editing this copy alone forks the
deliverable from what is actually being tested.

This file exists because Antigravity CLI (`agy`) and Codex load directory rules from `AGENTS.md`
walking up from the working directory; neither offers a setting that points at an arbitrary path.
